...
def execute_code(
    ...
    if not SANDBOX_AVAILABLE:
        return json.dumps({
            "error": "execute_code no está disponible en Windows. Usa llamadas de herramientas normales en su lugar."
        })

    if not code or not code.strip():
        return json.dumps({"error": "No se proporcionó código."})
    ...
    try:
        ...
        if status == "timeout":
            result["error"] = f"El script se agotó después de {timeout}s y fue terminado."
        elif status == "interrupted":
            result["output"] = stdout_text + "\n[execution interrupted — user sent a new message]"
        elif exit_code != 0:
            result["status"] = "error"
            result["error"] = stderr_text or f"El script salió con código {exit_code}"
...
def build_execute_code_schema(enabled_sandbox_tools: set = None) -> dict:
    """Construye el esquema execute_code con descripción listando solo herramientas habilitadas.

    Cuando las herramientas se deshabilitan vía ``hermes tools`` (p. ej. web se desactiva),
    la descripción del esquema NO debe mencionar web_search / web_extract —
    de lo contrario el modelo piensa que están disponibles y sigue intentando usarlas.
    """
    if enabled_sandbox_tools is None:
        enabled_sandbox_tools = SANDBOX_ALLOWED_TOOLS

    # Construye líneas de documentación de herramientas solo para herramientas habilitadas
    tool_lines = "\n".join(
        doc for name, doc in _TOOL_DOC_LINES if name in enabled_sandbox_tools
    )

    # Construye lista de importación de ejemplo desde herramientas habilitadas
    ...
    description = (
        "Ejecuta un script Python que puede llamar herramientas Hermes programáticamente. "
        "Usa esto cuando necesites 3+ llamadas de herramientas con lógica de procesamiento entre ellas, "
        "necesites filtrar/reducir salidas grandes de herramientas antes de que entren en tu contexto, "
        "necesites ramificación condicional (si X entonces Y si no Z), o necesites bucles "
        "(obtener N páginas, procesar N archivos, reintentar en fallo).\n\n"
        "Usa llamadas normales de herramientas en su lugar cuando: llamada de herramienta única sin procesamiento, "
        "necesites ver el resultado completo y aplicar razonamiento complejo, "
        "o la tarea requiera entrada interactiva del usuario.\n\n"
        f"Disponible vía `from hermes_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        "Límites:Timeout de 5 minutos, límite de 50KB stdout, máx 50 llamadas de herramientas por script. "
        "terminal() es solo en primer plano (sin fondo ni pty).\n\n"
        "Imprime tu resultado final a stdout. Usa stdlib de Python (json, re, math, csv, "
        "datetime, collections, etc.) para procesamiento entre llamadas de herramientas.\n\n"
        "También disponible (sin necesidad de importar — integrado en hermes_tools):\n"
        "  json_parse(text: str) — json.loads con strict=False; usa para salida de terminal() con caracteres de control\n"
        "  shell_quote(s: str) — shlex.quote(); usa cuando interpoles cadenas dinámicas en comandos shell\n"
        "  retry(fn, max_attempts=3, delay=2) — reintentar con backoff exponencial para fallos transitorios"
    )
        ...
        "parameters": {
            ...
            "properties": {
                "code": {
                    "type": "string",
                    "description": (
                        "Código Python a ejecutar. Importa herramientas con "
                        f"`from hermes_tools import {import_str}` "
                        "e impórtate tu resultado final a stdout."
                    ),
                },
            },
            "required": ["code"],

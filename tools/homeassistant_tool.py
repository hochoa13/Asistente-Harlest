...
HA_LIST_ENTITIES_SCHEMA = {
    ...
    "description": (
        "Lista entidades de Home Assistant. Opcionalmente filtra por dominio "
        "(luz, interruptor, clima, sensor, sensor_binario, cobertura, ventilador, etc.) "
        "o por nombre de área (sala de estar, cocina, dormitorio, etc.)."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "domain": {
                "type": "string",
                "description": (
                    "Dominio de entidad para filtrar (p. ej. 'light', 'switch', 'climate', "
                    "'sensor', 'binary_sensor', 'cover', 'fan', 'media_player'). "
                    "Omite para listar todas las entidades."
                ),
            },
            "area": {
                "type": "string",
                "description": (
                    "Nombre de área/cuarto para filtrar (p. ej. 'sala de estar', 'cocina'). "
                    "Coincide con los nombres amigables de entidades. Omite para listar todas."
                ),
            },
        },
        "required": [],
...
HA_GET_STATE_SCHEMA = {
    "name": "ha_get_state",
    "description": (
        "Obtiene el estado detallado de una única entidad de Home Assistant, incluyendo todos "
        "los atributos (brillo, color, temperatura de consigna, lecturas de sensor, etc.)."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "entity_id": {
                "type": "string",
                "description": (
                    "El ID de entidad a consultar (p. ej. 'light.living_room', "
...
HA_LIST_SERVICES_SCHEMA = {
    ...
    "description": (
        "Lista servicios de Home Assistant disponibles (acciones) para control de dispositivos. "
        "Muestra qué acciones se pueden realizar en cada tipo de dispositivo y qué "
        "parámetros aceptan. Úsalo para descubrir cómo controlar dispositivos "
        "encontrados vía ha_list_entities."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "domain": {
                "type": "string",
                "description": (
                    "Filtra por dominio (p. ej. 'light', 'climate', 'switch'). "
                    "Omite para listar servicios de todos los dominios."
                ),
            },
        },
        "required": [],
...
HA_CALL_SERVICE_SCHEMA = {
    ...
    "description": (
        "Llama un servicio de Home Assistant para controlar un dispositivo. Usa ha_list_services "
        "para descubrir servicios disponibles y sus parámetros para cada dominio."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "domain": {
                "type": "string",
                "description": (
                    "Dominio del servicio (p. ej. 'light', 'switch', 'climate', "
            ...
            "service": {
                "type": "string",
                "description": (
                    "Nombre del servicio (p. ej. 'turn_on', 'turn_off', 'toggle', "
            ...
            "entity_id": {
                "type": "string",
                "description": (
                    "ID de entidad destino (p. ej. 'light.living_room'). "
                    "Algunos servicios (como scene.turn_on) pueden no necesitar esto."
                ),
            },
            "data": {
                "type": "object",
                "description": (
                    "Datos de servicio adicionales. Ejemplos: "
                    '{"brightness": 255, "color_name": "blue"} para luces, '
                    '{"temperature": 22, "hvac_mode": "heat"} para clima, '
                    '{"volume_level": 0.5} para reproductores multimedia.'
                ),
            },
        },

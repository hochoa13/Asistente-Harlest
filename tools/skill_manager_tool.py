...
SKILL_MANAGE_SCHEMA = {
    "name": "skill_manage",
    "description": (
        "Gestiona habilidades (crear, actualizar, eliminar). Las habilidades son tu "
        "memoria de procedimientos — enfoques reutilizables para tipos de tareas recurrentes. "
        "Las nuevas habilidades van a ~/.hermes/skills/; las habilidades existentes se pueden modificar dónde vivan.\n\n"
        "Acciones: crear (SKILL.md completo + categoría opcional), "
        "parche (old_string/new_string — preferido para correcciones), "
        "editar (reescritura SKILL.md completa — solo para cambios mayores), "
        "eliminar, write_file, remove_file.\n\n"
        "Crear cuando: tarea compleja tuvo éxito (5+ llamadas), errores superados, "
        "enfoque corregido por usuario funcionó, flujo de trabajo no trivial descubierto, "
        "o usuario te pide que recuerdes un procedimiento.\n"
        "Actualizar cuando: instrucciones obsoletas/incorrectas, fallas específicas del SO, "
        "pasos faltantes o peligros encontrados durante el uso.\n\n"
        "Después de tareas difíciles/iterativas, ofrece guardar como una habilidad. "
        "Salta los de una sola vez simples. Confirma con el usuario antes de crear/eliminar.\n\n"
        "Buenas habilidades: condiciones de activación, pasos numerados con comandos exactos, "
        "sección de peligros, pasos de verificación. Usa skill_view() para ver ejemplos de formato."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "name": {
                "type": "string",
                "description": (
                    "Nombre de habilidad (minúsculas, guiones/guiones bajos, máx 64 caracteres). "
                    "Debe coincidir con una habilidad existente para parche/edición/eliminación/write_file/remove_file."
                )
            },
            "content": {
                "type": "string",
                "description": (
                    "Contenido SKILL.md completo (frontmatter YAML + cuerpo markdown). "
                    "Requerido para 'crear' y 'editar'. Para 'editar', lee primero la habilidad "
                    "con skill_view() y proporciona el texto completo actualizado."
                )
            },
            "old_string": {
                "type": "string",
                "description": (
                    "Texto a encontrar en el archivo (requerido para 'parche'). Debe ser único "
                    "a menos que replace_all=true. Incluye suficiente contexto circundante para "
                    "asegurar unicidad."
                )
            },
            "new_string": {
                "type": "string",
                "description": (
                    "Texto de reemplazo (requerido para 'parche'). Puede ser cadena vacía "
                    "para eliminar el texto coincidente."
                )
            },
            "replace_all": {
                "type": "boolean",
                "description": "Para 'parche': reemplaza todas las ocurrencias en lugar de requerir una coincidencia única (predeterminado: false)."
            },
            "category": {
                "type": "string",
                "description": (
                    "Categoría/dominio opcional para organizar la habilidad (p. ej., 'devops', "
                    "'data-science', 'mlops'). Crea un subgrupo de directorios. "
                    "Solo se usa con 'crear'."
                )
            },
            "file_path": {
                "type": "string",

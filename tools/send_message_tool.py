...
SEND_MESSAGE_SCHEMA = {
    "name": "send_message",
    "description": (
        "Envía un mensaje a una plataforma de mensajería conectada, u lista los objetivos disponibles.\n\n"
        "IMPORTANTE: Cuando el usuario pide enviar a un canal o persona específica "
        "(no solo un nombre de plataforma), llama primero a send_message(action='list') para ver "
        "objetivos disponibles, luego envía al correcto.\n"
        "Si el usuario solo dice un nombre de plataforma como 'enviar a telegram', envía directamente "
        "al canal principal sin listar primero."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "action": {
                "type": "string",
                "enum": ["send", "list"],
                "description": "Acción a realizar. 'send' (predeterminado) envía un mensaje. 'list' devuelve todos los canales/contactos disponibles en todas las plataformas conectadas."
            },
            "target": {
                "type": "string",
                "description": "Objetivo de entrega. Formato: 'plataforma' (usa canal principal), 'plataforma:#nombre-canal', 'plataforma:chat_id', o tema de Telegram 'telegram:chat_id:thread_id'. Ejemplos: 'telegram', 'telegram:-1001234567890:17585', 'discord:#bot-home', 'slack:#engineering', 'signal:+15551234567'"
            },
            "message": {
                "type": "string",

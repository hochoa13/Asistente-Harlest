...
TERMINAL_TOOL_DESCRIPTION = """Ejecuta comandos shell en un entorno Linux. El sistema de archivos persiste entre llamadas.

NO uses cat/head/tail para leer archivos — usa read_file en su lugar.
NO uses grep/rg/find para buscar — usa search_files en su lugar.
NO uses ls para listar directorios — usa search_files(target='files') en su lugar.
NO uses sed/awk para editar archivos — usa patch en su lugar.
NO uses echo/cat heredoc para crear archivos — usa write_file en su lugar.
Reserva terminal para: compilaciones, instalaciones, git, procesos, scripts, redes, administradores de paquetes y cualquier cosa que necesite un shell.

Foreground (defecto): Los comandos retornan INSTANTANEAMENTE cuando finaliza, incluso si el timeout es alto. Establece timeout=300 para compilaciones/scripts largos — seguirás obteniendo el resultado en segundos si es rápido. Prefiere foreground para todo lo que finalice.
Background: SOLO para servidores de larga duración, monitores o procesos que nunca se cierran. Establece background=true para obtener un session_id, luego usa process(action="wait") para bloquear hasta completarse — retorna instantáneamente al completarse, igual que foreground. Usa process(action="poll") solo cuando necesites una verificación de progreso sin bloqueo.
NO uses background para scripts, compilaciones o instalaciones — foreground con un timeout generoso siempre es mejor (menos llamadas de herramientas, resultados instantáneos).
Directorio de trabajo: Usa 'workdir' para cwd por comando.
Modo PTY: Establece pty=true para herramientas CLI interactivas (Codex, Claude Code, Python REPL).

NO uses vim/nano/herramientas interactivas sin pty=true — se cuelgan sin pseudo-terminal. Canaliza la salida de git a cat si podría paginar.
"""
...
TERMINAL_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            "command": {
                "type": "string",
                "description": "El comando a ejecutar en la VM"
            },
            "background": {
                "type": "boolean",
                "description": "SOLO para servidores/monitores que nunca se cierran. Para scripts, compilaciones, instalaciones — usa foreground con timeout en su lugar (retorna instantáneamente cuando se complete).",
                "default": False
            },
            "timeout": {
                "type": "integer",
                "description": "Máximo de segundos a esperar (defecto: 180). Retorna INSTANTANEAMENTE cuando el comando finaliza — establece alto para tareas largas, no esperarás innecesariamente.",
                "minimum": 1
            },
            "workdir": {
                "type": "string",
                "description": "Directorio de trabajo para este comando (ruta absoluta). Por defecto al directorio de trabajo de la sesión."
            },
            "check_interval": {
                "type": "integer",
                "description": "Segundos entre verificaciones automáticas de estado para procesos en segundo plano (solo gateway/mensajería, mínimo 30). Si se establece, reportaré progreso proactivamente.",
                "minimum": 30
            },
            "pty": {
                "type": "boolean",
                "description": "Ejecutar en modo pseudo-terminal (PTY) para herramientas CLI interactivas como Codex, Claude Code o Python REPL. Solo funciona con backends local y SSH. Por defecto: false.",
                "default": False
            }
        },
        "required": ["command"]

...
PROCESS_SCHEMA = {
    "name": "process",
    "description": (
        "Gestiona procesos en segundo plano iniciados con terminal(background=true). "
        "Acciones: 'list' (mostrar todos), 'poll' (verificar estado + salida nueva), "
        "'log' (salida completa con paginación), 'wait' (bloquear hasta terminar o timeout), "
        "'kill' (terminar), 'write' (enviar datos stdin sin salto de línea), "
        "'submit' (enviar datos + Enter, para responder prompts)."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "action": {
                ...
                "description": "Acción a realizar en procesos en segundo plano"
            },
            "session_id": {
                "type": "string",
                "description": "ID de sesión de proceso (de la salida de terminal en segundo plano). Requerido para todas las acciones excepto 'list'."
            },
            "data": {
                "type": "string",
                "description": "Texto a enviar a stdin del proceso (para acciones 'write' y 'submit')"
            },
            "timeout": {
                "type": "integer",
                "description": "Máximo de segundos para bloquear la acción 'wait'. Devuelve salida parcial en timeout.",
                "minimum": 1
            },
            "offset": {
                "type": "integer",
                "description": "Desplazamiento de línea para acción 'log' (predeterminado: últimas 200 líneas)"
            },
            "limit": {
                "type": "integer",
                "description": "Máximo de líneas a devolver para acción 'log'",
                "minimum": 1
            }
        },
        "required": ["action"]

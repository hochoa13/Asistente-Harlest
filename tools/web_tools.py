...
WEB_SEARCH_SCHEMA = {
    "name": "web_search",
    "description": "Buscar información en la web sobre cualquier tema. Devuelve hasta 5 resultados relevantes con títulos, URLs y descripciones.",
    "parameters": {
        "type": "object",
        "properties": {
            "query": {
...
WEB_EXTRACT_SCHEMA = {
    "name": "web_extract",
    "description": "Extrae contenido de URLs de páginas web. Devuelve el contenido de la página en formato markdown. También funciona con URLs de PDF (artículos de arxiv, documentos, etc.) — pasa el enlace del PDF directamente y se convierte a texto markdown. Las páginas menores de 5000 caracteres devuelven markdown completo; las páginas más grandes se resumen por IA y se limitan a ~5000 caracteres por página. Las páginas mayores de 2M se rechazan. Si una URL falla o agota el tiempo, usa la herramienta de navegador en su lugar.",
    "parameters": {
        "type": "object",
        "properties": {
            "urls": {
                "type": "array",
                "items": {"type": "string"},
                "description": "Lista de URLs para extraer contenido (máx. 5 URLs por llamada)",
                "maxItems": 5
            }
        },
        "required": ["urls"]

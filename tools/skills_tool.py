...
SKILLS_TOOL_DESCRIPTION = """Accede a documentos de habilidades que proporcionan instrucciones especializadas, directrices y conocimientos ejecutables.

Flujo de divulgación progresiva:
1. skills_list() - Devuelve metadatos (nombre, descripción, etiquetas, recuento_archivos_vinculados) para todas las habilidades
2. skill_view(nombre) - Carga contenido SKILL.md completo + muestra archivos_vinculados disponibles
3. skill_view(nombre, archivo_ruta) - Carga archivo vinculado específico (p. ej., 'referencias/api.md', 'scripts/train.py')

Las habilidades pueden incluir:
- referencias/: Documentación adicional, especificaciones de API, ejemplos
- plantillas/: Formatos de salida, archivos de configuración, código repetitivo
- activos/: Archivos complementarios (estándar agentskills.io)
- scripts/: Auxiliares ejecutables (scripts Python, shell)"""
...
SKILLS_LIST_SCHEMA = {
    "name": "skills_list",
    "description": "Enumera habilidades disponibles (nombre + descripción). Usa skill_view(nombre) para cargar contenido completo.",
    "parameters": {
        "type": "object",
        "properties": {
            "category": {
                "type": "string",
                "description": "Filtro de categoría opcional para estrechar resultados"
            }
        },
        "required": []
    }
...
SKILL_VIEW_SCHEMA = {
    "name": "skill_view",
    "description": "Las habilidades permiten cargar información sobre tareas y flujos de trabajo específicos, así como scripts y plantillas. Carga contenido completo de una habilidad o accede a sus archivos vinculados (referencias, plantillas, scripts). La primera llamada devuelve contenido SKILL.md más un dict 'linked_files' mostrando referencias/plantillas/scripts disponibles. Para acceder a esos, llama nuevamente con parámetro file_path.",
    "parameters": {
        "type": "object",
        "properties": {
            "name": {
                "type": "string",
                "description": "El nombre de la habilidad (usa skills_list para ver habilidades disponibles)"
            },
            "file_path": {
                "type": "string",
                "description": "OPCIONAL: Ruta a un archivo vinculado dentro de la habilidad (p. ej., 'referencias/api.md', 'plantillas/config.yaml', 'scripts/validate.py'). Omite para obtener contenido SKILL.md principal."
            }
        },
        "required": ["name"]
    }

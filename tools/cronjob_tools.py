...
Use for: reminders, periodic checks, scheduled reports, automated maintenance.""",
    "parameters": {
        ...
        "properties": {
            "prompt": {
                "type": "string",
                "description": "Instrucciones completas y autosuficientes. Debe incluir TODO el contexto - el agente futuro NO tendrá memoria de esta conversación."
            },
            "schedule": {
                "type": "string",
                "description": "Cuándo ejecutar: '30m' (una sola vez en 30min), 'every 30m' (recurrente), '0 9 * * *' (cron), o marca de tiempo ISO"
            },
            "name": {
                "type": "string",
                "description": "Nombre amigable opcional para el trabajo"
            },
            "repeat": {
                "type": "integer",
                "description": "Cuántas veces ejecutar. Omite para predeterminado (una sola vez para una sola vez, por siempre para recurrente). Establece a N para exactamente N ejecuciones."
            },
            "deliver": {
                "type": "string",
                "description": "Dónde enviar la salida: 'origin' (de vuelta a este chat), 'local' (solo archivos), 'telegram', 'discord', 'signal', o 'platform:chat_id'"
            }
        },
...
LIST_CRONJOBS_SCHEMA = {
    "name": "list_cronjobs",
    "description": """Enumera todos los trabajos cron programados con sus IDs, horarios y estado.

Utiliza esto para:
- Ver qué trabajos están actualmente programados
- Encontrar IDs de trabajo para eliminación con remove_cronjob
- Verificar estado de trabajo y tiempos de ejecución siguientes

Devuelve job_id, nombre, horario, estado de repetición, tiempos de última/próxima ejecución.""",
    "parameters": {
        "type": "object",
        "properties": {
            "include_disabled": {
                "type": "boolean",
                "description": "Incluir trabajos deshabilitados/completados en la lista (predeterminado: false)"
            }
        },
        "required": []
    }
...
REMOVE_CRONJOB_SCHEMA = {
    "name": "remove_cronjob",
    "description": """Eliminar un trabajo cron programado por su ID.

Utiliza list_cronjobs primero para encontrar el job_id del trabajo que deseas eliminar.
Los trabajos que han completado su recuento de repetición se eliminan automáticamente, pero puedes
usar esto para cancelar un trabajo antes de que se complete.""",
    "parameters": {
        "type": "object",
        "properties": {
            "job_id": {
                "type": "string",
                "description": "El ID del trabajo cron a eliminar (de la salida de list_cronjobs)"
            }
        },
        "required": ["job_id"]
    }

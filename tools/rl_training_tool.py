...
RL_LIST_ENVIRONMENTS_SCHEMA = {"name": "rl_list_environments", "description": "Enumera todos los entornos RL disponibles. Devuelve nombres de entornos, rutas y descripciones. CONSEJO: Lee file_path con herramientas de archivo para entender cómo funciona cada entorno (verificadores, carga de datos, recompensas).", "parameters": {"type": "object", "properties": {}, "required": []}}
RL_SELECT_ENVIRONMENT_SCHEMA = {"name": "rl_select_environment", "description": "Selecciona un entorno RL para entrenar. Carga la configuración predeterminada del entorno. Después de seleccionar, usa rl_get_current_config() para ver configuración y rl_edit_config() para modificarla.", "parameters": {"type": "object", "properties": {"name": {"type": "string", "description": "Nombre del entorno a seleccionar (de rl_list_environments)"}}, "required": ["name"]}}
RL_GET_CURRENT_CONFIG_SCHEMA = {"name": "rl_get_current_config", "description": "Obtiene la configuración actual del entorno. Devuelve solo campos que se pueden modificar: group_size, max_token_length, total_steps, steps_per_eval, use_wandb, wandb_name, max_num_workers.", "parameters": {"type": "object", "properties": {}, "required": []}}
RL_EDIT_CONFIG_SCHEMA = {"name": "rl_edit_config", "description": "Actualiza un campo de configuración. Usa rl_get_current_config() primero para ver todos los campos disponibles para el entorno seleccionado. Cada entorno tiene diferentes opciones configurables. Las configuraciones de infraestructura (tokenizer, URLs, lora_rank, learning_rate) están bloqueadas.", "parameters": {"type": "object", "properties": {"field": {"type": "string", "description": "Nombre del campo a actualizar (obtén campos disponibles de rl_get_current_config)"}, "value": {"description": "Nuevo valor para el campo"}}, "required": ["field", "value"]}}
RL_START_TRAINING_SCHEMA = {"name": "rl_start_training", "description": "Inicia una nueva ejecución de entrenamiento RL con el entorno y configuración actual. La mayoría de parámetros de entrenamiento (lora_rank, learning_rate, etc.) están fijos. Usa rl_edit_config() para establecer group_size, batch_size, wandb_project antes de iniciar. ADVERTENCIA: El entrenamiento toma horas.", "parameters": {"type": "object", "properties": {}, "required": []}}
RL_CHECK_STATUS_SCHEMA = {"name": "rl_check_status", "description": "Obtiene estado y métricas para una ejecución de entrenamiento. LIMITADO POR VELOCIDAD: aplica mínimo de 30 minutos entre comprobaciones para la misma ejecución. Devuelve métricas de WandB: step, state, reward_mean, loss, percent_correct.", "parameters": {"type": "object", "properties": {"run_id": {"type": "string", "description": "El ID de ejecución de rl_start_training()"}}, "required": ["run_id"]}}
RL_STOP_TRAINING_SCHEMA = {"name": "rl_stop_training", "description": "Detiene un trabajo de entrenamiento en ejecución. Usa si las métricas se ven mal, el entrenamiento se estanca, o quieres probar diferentes configuraciones.", "parameters": {"type": "object", "properties": {"run_id": {"type": "string", "description": "El ID de ejecución a detener"}}, "required": ["run_id"]}}
RL_GET_RESULTS_SCHEMA = {"name": "rl_get_results", "description": "Obtiene resultados finales y métricas para una ejecución de entrenamiento completada. Devuelve métricas finales y ruta al modelo entrenado.", "parameters": {"type": "object", "properties": {"run_id": {"type": "string", "description": "El ID de ejecución para obtener resultados"}}, "required": ["run_id"]}}
RL_LIST_RUNS_SCHEMA = {"name": "rl_list_runs", "description": "Enumera todos los entrenamientos (activos y completados) con su estado.", "parameters": {"type": "object", "properties": {}, "required": []}}
RL_TEST_INFERENCE_SCHEMA = {"name": "rl_test_inference", "description": "Prueba rápida de inferencia para cualquier entorno. Ejecuta algunos pasos de inferencia + puntuación usando OpenRouter. Predeterminado: 3 pasos x 16 finalizaciones = 48 ejecuciones por modelo, probando 3 modelos = 144 total. Prueba carga del entorno, construcción de prompt, análisis de inferencia, y lógica del verificador. Usa ANTES de entrenar para detectar problemas.", "parameters": {"type": "object", "properties": {"num_steps": {"type": "integer", "description": "Número de pasos a ejecutar (predeterminado: 3, máximo recomendado para pruebas)", "default": 3}, "group_size": {"type": "integer", "description": "Finalizaciones por paso (predeterminado: 16, como entrenamiento)", "default": 16}, "models": {"type": "array", "items": {"type": "string"}, "description": "Lista opcional de IDs de modelo de OpenRouter. Predeterminado: qwen/qwen3-8b, z-ai/glm-4.7-flash, minimax/minimax-m2.5"}}, "required": []}}

#!/usr/bin/env python3
"""
Hermes Agent CLI - Interfaz de Terminal Interactiva

Una interfaz de línea de comandos para Hermes Agent, inspirada en Claude Code.
Incluye arte ASCII, REPL interactivo, selección de toolsets y formato enriquecido.

Uso:
    python cli.py                          # Inicia modo interactivo con todas las herramientas
    python cli.py --toolsets web,terminal  # Inicia con toolsets específicos
    python cli.py -q "tu pregunta"         # Modo de consulta única
    python cli.py --list-tools             # Lista las herramientas disponibles y sale
"""
...
def _git_repo_root() -> Optional[str]:
    """Devuelve la raíz del repositorio git para el CWD o None si no hay repo."""
    import subprocess
    try:
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    """Crea un worktree de git aislado para esta sesión del CLI.

    Devuelve un dict con metadatos del worktree en caso de éxito, None en fallo.
    El dict contiene: path, branch, repo_root.
    """
    import subprocess
    ...
    print(f"\033[32m✓ Worktree creado:\033[0m {wt_path}")
...
def _cleanup_worktree(info: Dict[str, str] = None) -> None:
    """Elimina un worktree y su rama al salir.

    Si el worktree tiene cambios sin commitear, avisa y lo conserva.
    """
    ...
    if has_changes:
        print(f"\n\033[33m⚠ El worktree tiene cambios sin commitear, se conserva: {wt_path}\033[0m")
        print(f"  Para limpiarlo manualmente: git worktree remove {wt_path}")
    ...
    _active_worktree = None
    print(f"\033[32m✓ Worktree limpiado: {wt_path}\033[0m")


def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    """Elimina worktrees más antiguos que max_age_hours sin cambios pendientes.

    Se ejecuta en silencio al inicio para limpiar sesiones caídas/forzadas.
    """
    import subprocess
    import time
...
[bold #FFD700]║[/]  [#CD7F32]Messenger of the Digital Gods[/]    [dim #B8860B]Harlest[/]   [bold #FFD700]║[/]
[bold #FFD700]╚══════════════════════════════════════════════════════════════╝[/]
"""


def _build_compact_banner() -> str:
    """Construye un banner compacto que se ajuste al ancho actual del terminal."""
    w = min(shutil.get_terminal_size().columns - 2, 64)
    if w < 30:
        return "\n[#FFBF00]⚕ HARLEST HERMES[/] [dim #B8860B]- Harlest[/]\n"
    inner = w - 2  # inside the box border
    bar = "═" * w
    line1 = "⚕ NOUS HERMES - AI Agent Framework"
    line2 = "Messenger of the Digital Gods  ·  Harlest"
...
def _get_available_skills() -> Dict[str, List[str]]:
    """Escanea ~/.hermes/skills/ y devuelve las skills agrupadas por categoría.

    Returns:
        Dict que mapea nombre de categoría a lista de nombres de skills.
    """
    import os
...
def _format_context_length(tokens: int) -> str:
    """Da formato a un conteo de tokens para mostrar (ej. 128000 → '128K')."""
    if tokens >= 1_000_000:
...
def build_welcome_banner(console: Console, model: str, cwd: str, tools: List[dict] = None, enabled_toolsets: List[str] = None, session_id: str = None, context_length: int = None):
    """Construye e imprime un banner de bienvenida estilo Claude Code.

    Args:
        console: Instancia de Rich Console para imprimir
        model: Nombre del modelo actual (ej. "anthropic/claude-opus-4")
        cwd: Directorio de trabajo actual
        tools: Lista de definiciones de herramientas
        enabled_toolsets: Lista de nombres de toolsets habilitados
        session_id: Identificador único de sesión para logs
        context_length: Tamaño de ventana de contexto del modelo en tokens
    """
    ...
    left_lines.append(f"[{_accent}]{model_short}[/]{ctx_str} [dim {_dim}]·[/] [dim {_dim}]Harlest[/]")
    ...
    right_lines = []
    right_lines.append(f"[bold {_accent}]Herramientas disponibles[/]")
    ...
    if remaining_toolsets > 0:
        right_lines.append(f"[dim {_dim}](y {remaining_toolsets} toolsets más...)[/]")
    
    right_lines.append("")
    
    # Add skills section
    right_lines.append(f"[bold {_accent}]Skills disponibles[/]")
    ...
    if skills_by_category:
        ...
        right_lines.append(f"[dim {_dim}]No hay skills instaladas[/]")
    
    right_lines.append("")
    right_lines.append(f"[dim {_dim}]{len(tools)} tools · {total_skills} skills · /help para comandos[/]")
...
class HermesCLI:
    """CLI interactiva para Hermes Agent.

    Proporciona una interfaz REPL con formato enriquecido, historial de
    comandos y capacidades de ejecución de herramientas.
    """
    ...
    def __init__(
        ...
        """Inicializa el Hermes CLI.

        Args:
            model: Modelo a usar (por defecto: desde env o claude-sonnet)
            toolsets: Lista de toolsets a habilitar (por defecto: todos)
            provider: Proveedor de inferencia ("auto", "openrouter", "nous", "openai-codex", "zai", "kimi-coding", "minimax", "minimax-cn")
            api_key: API key (por defecto: desde el entorno)
            base_url: URL base de la API (por defecto: OpenRouter)
            max_turns: Iteraciones máximas de llamadas a herramientas compartidas con subagentes (por defecto: 90)
            verbose: Habilita logging detallado
            compact: Usa modo de visualización compacto
            resume: ID de sesión a reanudar (restaura historial desde SQLite)
            pass_session_id: Incluye el ID de sesión en el system prompt del agente
        """
    ...
    def _busy_command(self, status: str):
        """Muestra un estado de ocupado en la TUI mientras se ejecuta un comando slash."""
    ...
    def _ensure_runtime_credentials(self) -> bool:
        """Asegura que las credenciales en tiempo de ejecución estén listas antes de usar el agente.

        Vuelve a resolver las credenciales del proveedor para capturar rotación
        de llaves y refresh de tokens sin reiniciar el CLI.
        Devuelve True si las credenciales están listas, False si falla la autenticación.
        """
        ...
        if not isinstance(api_key, str) or not api_key:
            self.console.print("[bold red]El resolvedor de proveedor devolvió una API key vacía.[/]")
            return False
        if not isinstance(base_url, str) or not base_url:
            self.console.print("[bold red]El resolvedor de proveedor devolvió una base URL vacía.[/]")
            return False
    ...
    def _init_agent(self) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                _cprint(f"\033[1;31mSesión no encontrada: {self.session_id}{_RST}")
                _cprint(f"{_DIM}Usa un ID de sesión de una ejecución previa del CLI (hermes sessions list).{_RST}")
                return False
            ...
            if restored:
                ...
                _cprint(
                    f"{_GOLD}↻ Sesión reanudada {_BOLD}{self.session_id}{_RST}{_GOLD}{title_part} "
                    f"({msg_count} mensaje de usuario{'s' if msg_count != 1 else ''}, "
                    f"{len(restored)} mensajes en total){_RST}"
                )
            else:
                _cprint(f"{_GOLD}Sesión {self.session_id} encontrada pero sin mensajes. Iniciando desde cero.{_RST}")
        ...
        try:
            ...
            # Aplica cualquier título pendiente ahora que la sesión existe en la DB
            if self._pending_title and self._session_db:
                try:
                    self._session_db.set_session_title(self.session_id, self._pending_title)
                    _cprint(f"  Título de sesión aplicado: {self._pending_title}")
                    self._pending_title = None
                except (ValueError, Exception) as e:
                    _cprint(f"  No se pudo aplicar el título pendiente: {e}")
                    self._pending_title = None
            return True
        except Exception as e:
    ...
    def _preload_resumed_session(self) -> bool:
        """Carga temprano el historial de una sesión reanudada desde la base de datos.

        Se llama desde run() para que el historial esté disponible antes de que
        el usuario envíe su primer mensaje. Ajusta ``self.conversation_history``
        y muestra un estado en una sola línea. Devuelve True si se cargó el
        historial, False en caso contrario.

        El bloque correspondiente en ``_init_agent()`` comprueba si el historial
        ya está poblado y evita el viaje extra a la base de datos.
        """
        ...
        if not session_meta:
            self.console.print(
                f"[bold red]Sesión no encontrada: {self.session_id}[/]"
            )
            self.console.print(
                "[dim]Usa un ID de sesión de una ejecución previa del CLI "
        ...
        if restored:
            ...
            if session_meta.get("title"):
                title_part = f' "{session_meta["title"]}"'
            self.console.print(
                f"[#DAA520]↻ Sesión reanudada [bold]{self.session_id}[/bold]"
                f"{title_part} "
                f"({msg_count} mensaje de usuario{'s' if msg_count != 1 else ''}, "
                f"{len(restored)} mensajes en total)[/]"
            )
        else:
            self.console.print(
                f"[#DAA520]Sesión {self.session_id} encontrada pero sin "
                f"mensajes. Iniciando desde cero.[/]"
            )
            return False
    ...
    def _display_resumed_history(self):
        """Muestra un resumen compacto de mensajes previos de la conversación.

        Usa Rich con estilo tenue para que el resumen sea visualmente distinto
        de la conversación activa. Limita la vista a las últimas
        ``MAX_DISPLAY_EXCHANGES`` parejas usuario/asistente y muestra un
        indicador para mensajes anteriores ocultos.
        """
        ...
        def _strip_reasoning(text: str) -> str:
            """Elimina bloques <REASONING_SCRATCHPAD>...</REASONING_SCRATCHPAD>
            del texto mostrado (pensamientos internos del modelo)."""
            import re
            cleaned = re.sub(
        ...
        # Recoge entradas mostrables (omite system y mensajes de herramientas)
        ...
        for msg in self.conversation_history:
            ...
            if role == "user":
                ...
                if not parts:
                    # Omite mensajes solo de razonamiento sin salida visible
                    continue
        ...
        # Determina si es necesario truncar
        skipped = 0
        if len(entries) > MAX_DISPLAY_EXCHANGES * 2:
            skipped = len(entries) - MAX_DISPLAY_EXCHANGES * 2
            entries = entries[skipped:]

        # Construye la visualización usando Rich
        from rich.panel import Panel
        ...
        if skipped:
            lines.append(
                f"  ... {skipped} mensajes anteriores ...\n\n",
                style="dim italic",
            )

        for i, (role, text) in enumerate(entries):
            if role == "user":
                lines.append("  ● Tú: ", style="dim bold #DAA520")
                # Muestra la primera línea en línea, indenta el resto
        ...
        panel = Panel(
            lines,
            title="[dim #DAA520]Conversación anterior[/]",
    ...
    def _try_attach_clipboard_image(self) -> bool:
        """Revisa el portapapeles en busca de una imagen y la adjunta si existe.

        Guarda la imagen en ~/.hermes/images/ y agrega la ruta a
        ``_attached_images``. Devuelve True si se adjuntó una imagen.
        """
    ...
    def _handle_rollback_command(self, command: str):
        """Maneja /rollback — lista o restaura checkpoints del sistema de archivos."""
        from tools.checkpoint_manager import CheckpointManager, format_checkpoint_list

        if not hasattr(self, 'agent') or not self.agent:
            print("  No hay una sesión de agente activa.")
            return

        mgr = self.agent._checkpoint_mgr
        if not mgr.enabled:
            print("  Los checkpoints no están habilitados.")
            print("  Habilítalos con: hermes --checkpoints")
            print("  O en config.yaml: checkpoints: { enabled: true }")
            return
        ...
        if not arg:
            ...
            # Restaura por número o hash
            checkpoints = mgr.list_checkpoints(cwd)
            if not checkpoints:
                print(f"  No se encontraron checkpoints para {cwd}")
                return
            ...
            try:
                ...
                if 0 <= idx < len(checkpoints):
                    ...
                    print(f"  Número de checkpoint inválido. Usa 1-{len(checkpoints)}.")
                    return
            except ValueError:
                # Try as a git hash
                target_hash = arg
            ...
            if result["success"]:
                print(f"  ✅ Restaurado al checkpoint {result['restored_to']}: {result['reason']}")
                print(f"  Se guardó automáticamente un snapshot previo al rollback.")
            else:
                print(f"  ❌ {result['error']}")

    def _handle_paste_command(self):
        """Maneja /paste — revisa explícitamente el portapapeles en busca de una imagen.

        Es el fallback fiable para terminales donde BracketedPaste no se dispara
        con contenido de solo imagen (ej., VSCode terminal, Windows Terminal con WSL2).
        """
        from hermes_cli.clipboard import has_clipboard_image
        if has_clipboard_image():
            if self._try_attach_clipboard_image():
                n = len(self._attached_images)
                _cprint(f"  📎 Imagen #{n} adjuntada desde el portapapeles")
            else:
                _cprint(f"  {_DIM}(>_<) El portapapeles tiene una imagen pero la extracción falló{_RST}")
        else:
            _cprint(f"  {_DIM}(._.) No se encontró imagen en el portapapeles{_RST}")

    def _preprocess_images_with_vision(self, text: str, images: list) -> str:
        """Analiza imágenes adjuntas con la herramienta de visión y devuelve texto enriquecido.

        En vez de incrustar contenido base64 crudo de ``image_url`` en la
        conversación (que solo funciona con modelos con visión), este método
        preprocesa cada imagen con el modelo de visión auxiliar (Gemini Flash)
        y antepone las descripciones al mensaje del usuario, el mismo enfoque
        que usa el gateway de mensajería.

        Se incluye la ruta local del archivo para que el agente pueda
        reexaminar la imagen luego con ``vision_analyze`` si es necesario.
        """
        ...
        analysis_prompt = (
            "Describe con detalle todo lo que se ve en esta imagen. "
            "Incluye texto, código, datos, objetos, personas, disposición, colores "
            "y cualquier otra información visual relevante."
        )
        ...
        for img_path in images:
            ...
            _cprint(f"  {_DIM}👁️  analizando {img_path.name} ({size_kb}KB)...{_RST}")
            try:
                ...
                if result.get("success"):
                    ...
                    enriched_parts.append(
                        f"[El usuario adjuntó una imagen. Esto es lo que contiene:\n{description}]\n"
                        f"[Si necesitas verla más a fondo, usa vision_analyze con "
                        f"image_url: {img_path}]"
                    )
                    _cprint(f"  {_DIM}✓ imagen analizada{_RST}")
                else:
                    enriched_parts.append(
                        f"[El usuario adjuntó una imagen pero no se pudo analizar. "
                        f"Puedes intentar examinarla con vision_analyze usando "
                        f"image_url: {img_path}]"
                    )
                    _cprint(f"  {_DIM}⚠ el análisis de visión falló — se incluye la ruta para reintentar{_RST}")
            except Exception as e:
                enriched_parts.append(
        ...
        # Combina: descripciones de visión primero, luego el texto original del usuario
        user_text = text if isinstance(text, str) and text else ""
        if enriched_parts:
            prefix = "\n\n".join(enriched_parts)
            return f"{prefix}\n\n{user_text}" if user_text else prefix
        return user_text or "¿Qué ves en esta imagen?"

    def _show_tool_availability_warnings(self):
        """Muestra advertencias sobre herramientas deshabilitadas por falta de API keys."""
        try:
            from model_tools import check_tool_availability, TOOLSET_REQUIREMENTS
            ...
            if api_key_missing:
                self.console.print()
                self.console.print("[yellow]⚠️  Algunas herramientas deshabilitadas (faltan API keys):[/]")
                for item in api_key_missing:
                    tools_str = ", ".join(item["tools"][:2])  # Muestra las primeras 2 herramientas
                    if len(item["tools"]) > 2:
                        tools_str += f", +{len(item['tools'])-2} más"
                    self.console.print(f"   [dim]• {item['name']}[/] [dim italic]({', '.join(item['missing_vars'])})[/]")
                self.console.print("[dim]   Ejecuta 'hermes setup' para configurar[/]")
        except Exception:
            pass  # Don't crash on import errors
    
    def _show_status(self):
        """Muestra la barra de estado actual."""
        # Obtiene el conteo de herramientas
        tools = get_tool_definitions(enabled_toolsets=self.enabled_toolsets, quiet_mode=True)
        tool_count = len(tools) if tools else 0
        
        # Formatea el nombre del modelo (acorta si es necesario)
        model_short = self.model.split("/")[-1] if "/" in self.model else self.model
        if len(model_short) > 30:
            model_short = model_short[:27] + "..."
        
        # Obtiene el indicador de estado de API
        if self.api_key:
    ...
    def show_help(self):
        """Muestra información de ayuda con comandos categorizados."""
        from hermes_cli.commands import COMMANDS_BY_CATEGORY

        _cprint(f"\n{_BOLD}+{'-' * 55}+{_RST}")
        _cprint(f"{_BOLD}|{' ' * 14}(^_^)? Comandos disponibles{' ' * 15}|{_RST}")
        ...
        if _skill_commands:
            _cprint(f"\n  ⚡ {_BOLD}Comandos de Skills{_RST} ({len(_skill_commands)} instalados):")
            for cmd, info in sorted(_skill_commands.items()):
                _cprint(f"    {_GOLD}{cmd:<22}{_RST} {_DIM}-{_RST} {info['description']}")

        _cprint(f"\n  {_DIM}Tip: Solo escribe tu mensaje para chatear con Hermes.{_RST}")
        _cprint(f"  {_DIM}Multi-línea: Alt+Enter para una nueva línea{_RST}")
        _cprint(f"  {_DIM}Pegar imagen: Alt+V (o /paste){_RST}\n")
    
    def show_tools(self):
        """Muestra las herramientas disponibles con ASCII art kawaii."""
        tools = get_tool_definitions(enabled_toolsets=self.enabled_toolsets, quiet_mode=True)
        
        if not tools:
            print("(;_;) No hay herramientas disponibles")
            return
        
        # Header
        print()
        title = "(^_^)/ Herramientas disponibles"
        width = 78
        pad = width - len(title)
        print("+" + "-" * width + "+")
        ...
        print(f"  Total: {len(tools)} herramientas  ヽ(^o^)ノ")
        print()
    
    def show_toolsets(self):
        """Muestra los toolsets disponibles con ASCII art kawaii."""
        all_toolsets = get_all_toolsets()
        
        # Header
        print()
        title = "(^_^)b Toolsets disponibles"
        width = 58
        pad = width - len(title)
        print("+" + "-" * width + "+")
        print("|" + " " * (pad // 2) + title + " " * (pad - pad // 2) + "|")
        ...
        for name in sorted(all_toolsets.keys()):
            ...
            if info:
                ...
                # Marca si está habilitado actualmente
                marker = "(*)" if self.enabled_toolsets and name in self.enabled_toolsets else "   "
                print(f"  {marker} {name:<18} [{tool_count:>2} tools] - {desc}")
        
        print()
        print("  (*) = actualmente habilitado")
        print()
        print("  Tip: Usa 'all' o '*' para habilitar todos los toolsets")
        print("  Ejemplo: python cli.py --toolsets web,terminal")
        print()
    
    def show_config(self):
        """Muestra la configuración actual con ASCII art kawaii."""
        ...
        api_key_display = '********' + self.api_key[-4:] if self.api_key and len(self.api_key) > 4 else 'No configurada'
        ...
        if terminal_env == "ssh":
            ...
            print(f"  Destino SSH:  {ssh_user}@{ssh_host}:{ssh_port}")
        print(f"  Directorio:   {terminal_cwd}")
    ...
    def show_history(self):
        ...
        if not self.conversation_history:
            print("(._.) Aún no hay historial de conversación.")
            return
        ...
        def flush_tool_summary():
            ...
            noun = "mensaje" if hidden_tool_messages == 1 else "mensajes"
            print("\n  [Tools]")
            print(f"    ({hidden_tool_messages} mensajes de herramientas ocultos)")
            hidden_tool_messages = 0

        print()
        print("+" + "-" * 50 + "+")
        print("|" + " " * 12 + "(^_^) Historial de conversación" + " " * 7 + "|")
        print("+" + "-" * 50 + "+")
        ...
        for msg in self.conversation_history:
            ...
            if content_text:
                ...
                noun = "llamada" if tool_count == 1 else "llamadas"
                preview = f"(se solicitaron {tool_count} herramientas {noun})"
                suffix = ""
            else:
                preview = "(sin respuesta de texto)"
                suffix = ""
    ...
    def reset_conversation(self):
        ...
        print("(^_^)b ¡Historial reiniciado!")
    
    def save_conversation(self):
        """Guarda la conversación actual en un archivo."""
        if not self.conversation_history:
            print("(;_;) No hay conversación para guardar.")
            return
        ...
        try:
            ...
            print(f"(^_^)v Conversación guardada en: {filename}")
        except Exception as e:
            print(f"(x_x) Error al guardar: {e}")
    
    def retry_last(self):
        """Reintenta el último mensaje del usuario eliminando el último intercambio.

        Elimina la última respuesta del asistente (y cualquier mensaje de herramientas)
        y el último mensaje del usuario, luego reenvía ese mensaje al agente.
        Devuelve el mensaje a reenviar o None si no hay nada que reintentar.
        """
        if not self.conversation_history:
            print("(._.) No hay mensajes para reintentar.")
            return None
        ...
        if last_user_idx is None:
            print("(._.) No se encontró mensaje de usuario para reintentar.")
            return None
        ...
        print(f"(^_^)b Reintentando: \"{last_message[:60]}{'...' if len(last_message) > 60 else ''}\"")
        return last_message
    
    def undo_last(self):
        """Elimina el último intercambio usuario/asistente del historial.

        Recorre hacia atrás y elimina todos los mensajes desde el último
        mensaje de usuario en adelante (incluyendo respuestas, herramientas, etc.).
        """
        if not self.conversation_history:
            print("(._.) No hay mensajes para deshacer.")
            return
        ...
        if last_user_idx is None:
            print("(._.) No se encontró mensaje de usuario para deshacer.")
            return
        ...
        print(f"(^_^)b Se deshicieron {removed_count} mensaje(s). Eliminado: \"{removed_msg[:60]}{'...' if len(removed_msg) > 60 else ''}\"")
        remaining = len(self.conversation_history)
        print(f"  {remaining} mensaje(s) restantes en el historial.")
    
    def _show_model_and_providers(self):
        """Vista unificada para /model y /provider.

        Muestra el modelo y proveedor actuales, luego lista todos los
        proveedores autenticados con sus modelos disponibles para facilitar
        el cambio.
        """
        ...
        print(f"\n  Actual: {self.model} vía {current_label}")
        print()
        ...
        if authed:
            print("  Proveedores autenticados y modelos:")
            for p in authed:
                ...
                if curated:
                    ...
                    print(f"      (usa /model {p['id']}:<model-name>)")
                print()

        if unauthed:
            names = ", ".join(p["label"] for p in unauthed)
            print(f"  No configurados: {names}")
            print(f"  Ejecuta: hermes setup")
            print()

        print("  Cambiar modelo:      /model <model-name>")
        print("  Cambiar proveedor:   /model <provider>:<model-name>")
        if authed and len(authed) > 1:
            ...
            if other_models:
                example_model = other_models[0][0]
                    print(f"  Ejemplo: /model {other['id']}:{example_model}")

    def _handle_prompt_command(self, cmd: str):
        """Maneja el comando /prompt para ver o ajustar el system prompt."""
        ...
        if len(parts) > 1:
            ...
            if new_prompt.lower() == "clear":
                ...
                if save_config_value("agent.system_prompt", ""):
                    print("(^_^)b System prompt limpiado (guardado en config)")
                else:
                    print("(^_^) System prompt limpiado (solo sesión)")
            else:
                self.system_prompt = new_prompt
                self.agent = None  # Force re-init
                if save_config_value("agent.system_prompt", new_prompt):
                    print(f"(^_^)b System prompt establecido (guardado en config)")
                else:
                    print(f"(^_^) System prompt establecido (solo sesión)")
            ...
            if self.system_prompt:
                ...
                print("  (no hay prompt personalizado - usando el predeterminado)")
            print()
            print("  Uso:")
            print("    /prompt <texto>  - Establece un system prompt personalizado")
            print("    /prompt clear    - Elimina el prompt personalizado")
            print("    /personality     - Usa una personalidad predefinida")
            print()
    ...
    def _handle_personality_command(self, cmd: str):
        """Maneja el comando /personality para establecer personalidades predefinidas."""
        parts = cmd.split(maxsplit=1)
        ...
        if len(parts) > 1:
            ...
            if personality_name in ("none", "default", "neutral"):
                ...
                self.agent = None  # Force re-init
                if save_config_value("agent.system_prompt", ""):
                    print("(^_^)b Personalidad limpiada (guardada en config)")
                else:
                    print("(^_^) Personalidad limpiada (solo sesión)")
                print("  Sin personalidad adicional — usando el comportamiento base del agente.")
            elif personality_name in self.personalities:
                self.system_prompt = self._resolve_personality_prompt(self.personalities[personality_name])
                self.agent = None  # Force re-init
                if save_config_value("agent.system_prompt", self.system_prompt):
                    print(f"(^_^)b Personalidad establecida a '{personality_name}' (guardada en config)")
                else:
                    print(f"(^_^) Personalidad establecida a '{personality_name}' (solo sesión)")
                print(f"  \"{self.system_prompt[:60]}{'...' if len(self.system_prompt) > 60 else ''}\"")
            else:
                print(f"(._.) Personalidad desconocida: {personality_name}")
                print(f"  Disponibles: none, {', '.join(self.personalities.keys())}")
        else:
            # Show available personalities
            print()
            print("+" + "-" * 50 + "+")
            print("|" + " " * 12 + "(^o^)/ Personalidades" + " " * 13 + "|")
            print("+" + "-" * 50 + "+")
            print()
            print(f"  {'none':<12} - (sin personalidad adicional)")
            ...
            print()
            print("  Uso: /personality <nombre>")
            print()
    
    def _handle_cron_command(self, cmd: str):
        """Maneja el comando /cron para administrar tareas programadas."""
        ...
        if len(parts) == 1:
            ...
            print()
            print("+" + "-" * 60 + "+")
            print("|" + " " * 18 + "(^_^) Tareas programadas" + " " * 17 + "|")
            print("+" + "-" * 60 + "+")
            print()
            print("  Comandos:")
            print("    /cron                     - Lista trabajos programados")
            print("    /cron list                - Lista trabajos programados")
            print('    /cron add <schedule> <prompt>  - Agrega un nuevo trabajo')
            print("    /cron remove <job_id>     - Elimina un trabajo")
            print()
            print("  Formatos de schedule:")
            print("    30m, 2h, 1d              - Retraso único")
            print('    "every 30m", "every 2h"  - Intervalo recurrente')
            print('    "0 9 * * *"              - Expresión cron')
            print()
            
            # Show current jobs
            jobs = list_jobs()
            if jobs:
                print("  Trabajos actuales:")
                print("  " + "-" * 55)
                for job in jobs:
                    ...
                    if job.get("next_run_at"):
                        ...
                        print(f"      Próxima: {next_run.strftime('%Y-%m-%d %H:%M')}")
                    print()
            else:
                    print("  No hay trabajos programados. Usa '/cron add' para crear uno.")
            print()
            return
        ...
        if subcommand == "list":
            ...
            jobs = list_jobs()
            if not jobs:
                print("(._.) No hay trabajos programados.")
                return
